...
function App() {
  ...
  const [profile,setProfile] = useState(() => {
     try { return JSON.parse(localStorage.getItem('hf2_profile') || '{"name":"","handle":""}') }
  catch { return { name: '', handle: '' } }
  })
...
function handleToggle(id) { setRecords(p => toggleRecord(p, id, todayKey(), 'done')) }
function handleSkip(id)   { setRecords(p => toggleRecord(p, id, todayKey(), 'skip')) }
...
function handleAdd(name, emoji) {
  setHabits(p => [...p, { id: Date.now().toString(), name, emoji, createdAt: todayKey() }])
  setModalOpen(false)
}

function handleSaveProfile(name, handle) { setProfile({ name, handle }) } 
  ...
  return (
    <div className="app">
      ...
      { activeTab === 'today'   && <TodayPage habits={habits} records={records} onToggle={handleToggle} onSkip={handleSkip} onDelete={handleDelete} onOpenModal={()=> setModalOpen(true)} /> }
{/* { activeTab === 'today' && <TodayPage habits={habits} records={records} onToggle={() => {}} onSkip={() => {}} onDelete={() => {}} onOpenModal={() => {}} /> } */}
      { activeTab === 'stats'   && <StatsPage habits={habits} records={records} /> }
